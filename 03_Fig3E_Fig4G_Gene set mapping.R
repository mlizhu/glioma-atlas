...
##OD supplements cells without the highest gene expression:  Mφ
cluster.ave_1 <- cluster.ave %>% rownames_to_column(var = "signature") 
Def<- cluster.ave_1[,c(1,6)]
df4 <- left_join(df3,Def,by="signature")
rownames(df4) <- df4$signature
df4 <- df4[,c(-1,-2)]
ph_83  <- pheatmap::pheatmap(df4,
                          border_color = NA,
                          border=F,
                          color = colours,
                          scale = "row",
